...
//console.log(absolutePath);
  const nameMappings = ['lizzo', 'john', 'zakaria'];
  var user = "";
// for middleware
...
app.get('/login', function (req, res) {
    ...
    // test if input is valid, if so, next phase
      ...
      for(var i = 0 ;i< nameMappings.length ; i++){
        if (response.username == nameMappings[i])
            user = response.username ;
      }
    if (response.username == user || response.password == "bido") {
...
app.post('/upload', async (req, res, next) => {
    ...
    try {
        ...
        setTimeout(function() {
            if (confValue < 90.0 && imgLabel == user) {
...
async function checkFace() {
    ...
    //const nameMappings = ['hafsa', 'john', 'zakaria','lizzo'];
